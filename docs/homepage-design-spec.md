...
- **Page:** Homepage
- **Status:** Historical design brief (implemented). For the live code map, see [`architecture.md`](architecture.md).
...
This document records the approved visual direction used during the redesign. Where the shipped theme differs (e.g. demo fallbacks, booking demo modal), prefer the code and [`architecture.md`](architecture.md).
...
Use ACF Free gallery slots on the front page (`gallery_*` fields) assembled by `sw_get_gallery_pairs()`.
...
Field names live in `inc/acf-fields.php` and clinic options in `inc/clinic-settings.php`.
...
See [`architecture.md`](architecture.md) for the shipped structure.
