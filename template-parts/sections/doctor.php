...
$featured = function_exists( 'get_field' ) ? get_field( 'featured_team_member', sw_front_page_id() ) : null;
$doctor   = null;
...
if ( ! $team_url ) {
	...
	$team_url = $about ? get_permalink( $about ) : get_permalink( $doctor );
}

$portrait_id = (int) get_post_thumbnail_id( $doctor->ID );
// Fallback: featured image set on the homepage (common editor mistake).
if ( ! $portrait_id ) {
	$portrait_id = (int) get_post_thumbnail_id( sw_front_page_id() );
}
?>
<section class="sw-doctor-feature" id="zespol" aria-labelledby="sw-doctor-feature-heading">
