...
$cta2_link  = get_field( 'hero_cta_secondary_link' ) ?: '#uslugi';

$has_media     = ! empty( $hero_image );
$section_class = 'sw-hero';
?>
<section class="<?php echo esc_attr( $section_class ); ?>" aria-labelledby="sw-hero-heading">
	<div class="sw-hero__layout">
		...
		<div class="sw-hero__media<?php echo $has_media ? '' : ' sw-hero__media--placeholder'; ?>">
			<?php if ( $has_media ) : ?>
				<?php sw_image( $hero_image, 'sw-hero', true, array( 'class' => 'sw-hero__image' ) ); ?>
			<?php else : ?>
				<span class="sw-visually-hidden"><?php esc_html_e( 'Obraz hero w przygotowaniu', 'stomatologia-wiacek' ); ?></span>
			<?php endif; ?>
		</div>
	</div>
</section>
